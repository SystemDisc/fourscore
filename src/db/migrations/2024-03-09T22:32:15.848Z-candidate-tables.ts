...
export async function up(db: Kysely<any>) {
  ...
  let me = await db.selectFrom('User')
    .selectAll()
    .where('User.email', '=', 'admin@zornco.com')
    .executeTakeFirst();

  if (!me) {
    const [me] = await db.insertInto('User')
      .values({
        name: 'Timothy Zorn',
  ...
  const federal = await db.selectFrom('Locality')
    .selectAll()
    .where('name', '=', 'Federal')
    .executeTakeFirst();

  const [office] = await db.insertInto('Office')
    .values({
  ...
  await db.insertInto('CandidateOffice')
    .values({
      userId: me!.id,
      officeId: office.id,
    })
