...
import { CSSProperties, ComponentType, ReactNode, Ref, forwardRef, useCallback, useEffect, useImperativeHandle, useRef, useState } from 'react';
...
export default forwardRef(function Accordion({
  ...
  const Icon: ComponentType<{className: string; style?: CSSProperties}> = icon || BsChevronDown;
        ...
        <div className='flex items-center'>
          {tabHeadings &&
            tabHeadings.map((heading, index) => (
              <Button key={heading} type='button' className={classNames('tab-button inline-block border border-sermons-dark rounded font-bold px-2 py-1 mr-2', {
                'bg-white !text-black active:brightness-125 hover:brightness-110': selectedTab !== index,
                'bg-gradient-to-b from-sermons-light to-sermons-dark !text-black active:from-sermons-dark active:to-sermons-dark hover:brightness-110': selectedTab === index,
              })} onClick={() => setSelectedTab(index)}>{heading}</Button>
            ))
          }
          <Icon className='inline-block text-3xl transition-transform text-sermons-dark' style={{transform: open ? `rotate(${iconRotation}deg)` : ''}} />
        </div>
      </header>
      <article className={classNames('transition-[height] bg-white text-black', {
