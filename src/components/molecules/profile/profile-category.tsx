...
export default function ProfileCategory({ id, categoryId }: { id?: string; categoryId?: string }) {
  ...
  const AnswerElement = (answer: Answer) => {
    return (
      <div className='flex flex-col gap-1 py-4 font-light'>
        ...
        >
          <div className='text-lg text-white'>{answer.question}</div>
        </div>
        <div className='p-2'>
          <div className='text-lg font-normal'>{dateToFormattedString(answer.dateUpdated)}</div>
          <ol className='relative border-s border-gray-200 dark:border-gray-700'>
            <li className='my-2 ms-4'>
              <div
                ...
                style={{
                  background: answer.agree
                    ? 'linear-gradient(0deg, #69F7A5, #22C064'
                    : 'linear-gradient(0deg, #7E8287, #474B4F',
                }}
              ></div>
              <div>
                <div className='text-lg'>{answer.agree ? 'Yes' : 'No'}</div>
                {answer.notes && <div className='text-lg font-light'>&ldquo;{answer.notes}&rdquo;</div>}
              </div>
            </li>
          </ol>
        </div>
  ...
  return (
    <div className='flex flex-col justify-between gap-2 p-4'>
      ...
      <div className='p-4'>
        <div className='text-2xl font-light'>{category}</div>
        <div className='flex flex-row gap-2 items-center'>
          <div className='text-lg font-light flex items-center'>{Math.round(score || 0)}% Similar</div>
          <Star rate={calculateRateFromScore(score)} />
        </div>
      </div>

      {loading && <SingleSkeleton />}
      {answers?.map((answer: Answer) => <div key={answer.id}>{AnswerElement(answer)}</div>)}
    </div>
  );
}
